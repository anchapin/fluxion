use fluxion::sim::construction::{
    Assemblies, Construction, ConstructionLayer, exterior_film_coeff, interior_film_coeff,
};

fn main() {
    // Example 1: Create a custom construction layer
    let fiberglass = ConstructionLayer::new(0.04, 12.0, 840.0, 0.066);
    println!("Fiberglass layer R-value: {:.4} m²K/W", fiberglass.r_value());
    println!("Fiberglass thermal capacitance: {:.2} J/m²K", 
    ...
    println!("  Total R-value: {:.4} m²K/W", custom_wall.r_value_total(None));
    println!("  U-value: {:.4} W/m²K", custom_wall.u_value(None));
    println!("  Thermal capacitance: {:.2} J/m²K", 
    ...
    println!("  U-value: {:.4} W/m²K (expected ~0.514)", 
             case_600_wall.u_value(None));
    println!("  R-value: {:.4} m²K/W", 
             case_600_wall.r_value_total(None));
    ...
    println!("  R-value: {:.4} m²K/W", 
             case_900_wall.r_value_total(None));
    println!("  Thermal capacitance: {:.2} J/m²K", 
    ...
    for wind_speed in [0.0, 2.0, 5.0, 10.0] {
        ...
        println!("  {:.1} m/s wind → h_ext = {:.2} W/m²K → U = {:.4} W/m²K",
                 wind_speed, h_ext, u);
    }
    ...
    println!("  Exterior (h_ext) at 3.5 m/s: {:.2} W/m²K", 
             exterior_film_coeff(3.5));

    // Example 6: All ASHRAE 140 assemblies
    println!("\nASHRAE 140 Assembly U-values:");
    println!("  Low mass wall: {:.4} W/m²K", Assemblies::low_mass_wall().u_value(None));
    println!("  Low mass roof: {:.4} W/m²K", Assemblies::low_mass_roof().u_value(None));
    println!("  Insulated floor: {:.4} W/m²K", Assemblies::insulated_floor().u_value(None));
    println!("  High mass wall: {:.4} W/m²K", Assemblies::high_mass_wall().u_value(None));
    println!("  High mass roof: {:.4} W/m²K", Assemblies::high_mass_roof().u_value(None));
}

...
fn main() {
    ...
    println!("Window Ratio: {:?}", model.setpoints.window_ratio);
    println!("Window U-value: {}", model.solar.window_u_value);
    ...
    println!(
        "Solar Gains[0]: {:.2} W/m2",
        model.solar.solar_gains.as_ref()[0]
    );
    let floor_area = model.setpoints.zone_area.as_ref()[0];
    println!(
        "Total Solar Gain: {:.2} W",
        model.solar.solar_gains.as_ref()[0] * floor_area
    );

    // Check surfaces
    if let Some(surfaces) = model.solar.surfaces.first() {

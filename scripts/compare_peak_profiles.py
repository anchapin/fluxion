...
def load_reference_data(json_path):
    ...
    if len(heating_j) < num_raw: heating_j = np.zeros(num_raw)
    if len(cooling_j) < num_raw: cooling_j = np.zeros(num_raw)
...
def plot_peak_days(flux_hours, flux_hvac, ref_hours, ref_hvac, output_prefix):
    ...
    for title, start, end in days:
        ...
        plt.plot(ref_hours[start:end], ref_hvac[start:end], 'r--', label='Reference (EnergyPlus)')
...
def main():
    parser = argparse.ArgumentParser(description='Compare Fluxion vs Reference peak profiles.')
    parser.add_argument('--fluxion', type=str, default='case_900_peak_hourly.csv', help='Path to Fluxion CSV')
    parser.add_argument('--reference', type=str, default='refdata/energyplus_reference/900_reference.json', help='Path to Reference JSON')
    parser.add_argument('--output', type=str, default='peak_comparison', help='Prefix for output plots')

"""
Pytest harness for ``scripts/verify_gauge_solver_regression.py`` -- Issue #3070.

Mirrors the ``load_script`` + ``tmp_path`` mock-repo pattern from
``scripts/ci/test_check_strict_energy_gate_regression.py``:
...
SCRIPT_NAME = "verify_gauge_solver_regression"
...
def verifier(load_script):
    """Freshly-loaded copy of the gauge-solver regression verifier."""
...
def test_load_snapshot_set_parses_manifest(verifier, tmp_path):
    ...
    payloads = {
        "case_195": _make_case_payload(
            case_id="195",
            heating=3.95, cooling=0.28,
            peak_h=1.05, peak_c=0.0,
        ),
        "case_600": _make_case_payload(
            case_id="600",
            heating=4.6, cooling=3.3,
            peak_h=4.4, peak_c=3.7,
        ),
        "case_620": _make_case_payload(
            case_id="620",
            heating=5.0, cooling=2.4,
            peak_h=3.2, peak_c=2.9,
        ),
    }
    ...
    assert snapshot_set.cases["case_600"].metrics["annual_heating_mwh"] == 4.6
...
def test_load_snapshot_set_rejects_wrong_schema_version(verifier, tmp_path):
    ...
    payloads = {
        "case_195": _make_case_payload(
            case_id="195", heating=1.0, cooling=0.0, peak_h=1.0, peak_c=0.0,
        ),
    }
    snapshot_dir = _write_snapshot_set(tmp_path, payloads)
...
def test_load_snapshot_set_skips_doc_keys(verifier, tmp_path):
    ...
    Mirrors the layout of ``tests/reference_data/gauge_solver_baseline/baseline_manifest.json``
    which plants a ``cases._doc`` string next to the case entries. The
    loader must skip these without crashing (regression guard for the
    earlier ``'str' object has no attribute 'get'`` bug).
    """
    payloads = {
        "case_195": _make_case_payload(
            case_id="195", heating=1.0, cooling=0.0, peak_h=1.0, peak_c=0.0,
...
def test_snapshot_set_placeholder_detection(verifier, tmp_path):
    ...
    payload = _make_case_payload(
        case_id="195",
        heating=None, cooling=None, peak_h=None, peak_c=None,
    )
...
def test_compute_diff_no_drift(verifier, tmp_path):
    ...
    payload = _make_case_payload(
        case_id="600", heating=5.0, cooling=5.0, peak_h=5.0, peak_c=5.0,
    )
...
def test_compute_diff_flags_regression_when_delta_exceeds_tolerance(verifier, tmp_path):
    ...
    after_payload = _make_case_payload(
        case_id="600", heating=5.001, cooling=5.0, peak_h=5.0, peak_c=5.0,
    )
    ...
    heating_delta = next(d for d in report.deltas if d.metric == "annual_heating_mwh")
    assert heating_delta.delta == pytest.approx(0.001)
    assert not heating_delta.within_tolerance
...
def test_compute_diff_respects_override_tolerance(verifier, tmp_path):
    ...
    after_payload = _make_case_payload(
        case_id="600", heating=5.0009, cooling=5.0, peak_h=5.0, peak_c=5.0,
    )
    before_dir = _write_snapshot_set(tmp_path / "before", {"case_600": before_payload})
    after_dir = _write_snapshot_set(tmp_path / "after", {"case_600": after_payload})
    ...
    report = verifier.compute_diff(
        before_set,
        after_set,
        override_tolerance={"annual_heating_mwh": 0.001},
    )

    assert not report.has_regression
    heating_delta = next(d for d in report.deltas if d.metric == "annual_heating_mwh")
    assert heating_delta.within_tolerance
    assert heating_delta.tolerance == 0.001
...
def test_compute_diff_records_schema_drift_when_case_missing(verifier, tmp_path):
    ...
    before_dir = _write_snapshot_set(
        tmp_path / "before",
        {"case_600": _make_case_payload(case_id="600", heating=1.0, cooling=1.0, peak_h=1.0, peak_c=1.0)},
    )
    after_dir = _write_snapshot_set(
        tmp_path / "after",
        {
            "case_600": _make_case_payload(case_id="600", heating=1.0, cooling=1.0, peak_h=1.0, peak_c=1.0),
            "case_620": _make_case_payload(case_id="620", heating=1.0, cooling=1.0, peak_h=1.0, peak_c=1.0),
        },
    )
...
def test_main_exits_one_on_regression(verifier, tmp_path, capsys):
    ...
    assert "FAIL" in out
    assert "annual_heating_mwh" in out
...
def test_main_returns_two_when_snapshot_unpopulated(verifier, tmp_path, capsys):
    ...
    placeholder = _make_case_payload(
        case_id="600", heating=None, cooling=None, peak_h=None, peak_c=None,
...
def test_main_tolerance_override_cli(verifier, tmp_path, capsys):
    ...
    rc, _ = _invoke(
        verifier, before_dir, after_dir,
        "--allow-placeholder",
        "--tolerance", "annual_heating_mwh=0.001",
    )
    assert rc == verifier.EXIT_OK
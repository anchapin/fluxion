#!/usr/bin/env python3
"""
TDD snapshot diff verifier for the ``h_tr_em`` per-step recompute.

Issue #3063 — PR #3024 (issue #2891) introduced wind-velocity-dependent
exterior convection (``h_se``) in the 5R1C path (ASHRAE 140 §5.2.6) but
left the sister ``h_tr_em`` (envelope-to-mass conductance) time-invariant
at build-time ``1/EXTERIOR_FILM_COEFF_DEFAULT``. The sub-agent report that
closed PR #3024 explicitly noted: *"h_tr_em (envelope-to-mass conductance)
remains time-invariant."* That is the issue body of #3063.

Per AGENTS.md ("do NOT modify physics code without checking ARCHITECTURE.md
first"), RULES.md ("no parameter tuning", "must-never hardcode results"),
and the issue's own scope, the per-step recompute is out of scope for a
single sub-agent. Per the issue's "Recommended Direction," the closure
path is:

  1. Extend ``HvacState`` or ``MassState`` with ``h_tr_em_zone: Vec<f64>``
     (per-zone, per-timestep).
  2. Recompute at every timestep in ``step_physics_5r1c`` via
     ``physics::exterior_convection::h_c_ext_wind_dependent``.
  3. Update the ``EnergyPlus-equivalent baseline`` invariant check.

This script is the verifier half of the TDD contract that ADR-0009 ships:
  ...
  * Refuses to operate on placeholder snapshots (any ``metrics.*`` field
    is ``null``) — fails closed with exit code 2 so a future recompute
  ...
  * Reports the diff as JSON (``--json``) or as a human-readable table
    ...
    convention used by ``scripts/verify_gauge_solver_regression.py`` /
...
Usage::
    ...
    python3 scripts/verify_h_tr_em_regression.py \
        ...
        --after  path/to/post-recompute-snapshots \
        [--json] [--strict]
...
def render_text_report(report: DiffReport) -> str:
    ...
    lines = [
        "=" * 72,
        "H_TR_EM WIND-DEPENDENT REGRESSION VERIFIER (Issue #3063 / PR #3024)",
        "=" * 72,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "TDD snapshot diff verifier for the h_tr_em per-step recompute "
            "(Issue #3063 / PR #3024 / Issue #2891). Fails closed when ANY "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "key=value list (e.g. 'annual_heating_kwh=0.001,annual_cooling_kwh=0.001'). "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "verifier fails closed (exit 2) so a future recompute run "
    ...
    if not args.allow_placeholder:
        if before_set.is_placeholder() or after_set.is_placeholder():
            ...
            print(
                ...
                + "docs/adr/0009-h-tr-em-wind-dependent.md). "
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp_str:
        ...
        after_dir = tmp / "after"
        _populate_synthetic_snapshot(before_dir, metric_value=6250.0)
        _populate_synthetic_snapshot(after_dir, metric_value=6250.0)
        ...
        # 2) Inject a cooling regression in --after and verify EXIT_REGRESSION
        _populate_synthetic_snapshot(
            after_dir, metric_value=6250.0, metric_value_cooling=758.0
        )
        rc_regression = main([
            "--before", str(before_dir),
            "--after", str(after_dir),
        ...
        if rc_regression != EXIT_REGRESSION:
            failures.append(
                f"synthetic cooling drift: expected EXIT_REGRESSION, got {rc_regression}"
            )
        ...
        # 5) Tolerance override absorbs cooling drift (annual_cooling_kwh = 550 → 758 with tol 250)
        _populate_synthetic_snapshot(before_dir, metric_value=6250.0, metric_value_cooling=550.0)
        _populate_synthetic_snapshot(after_dir, metric_value=6250.0, metric_value_cooling=758.0)
        rc_tol = main([
            "--before", str(before_dir),
            "--after", str(after_dir),
            "--allow-placeholder",
        ...
        if rc_tol != EXIT_OK:
            failures.append(
                f"tolerance override: expected EXIT_OK, got {rc_tol}"
            )
...
def _populate_synthetic_snapshot(
    directory: Path,
    *,
    metric_value: float,
    metric_value_cooling: float | None = None,
) -> None:
    """Write a 1-case synthetic snapshot set for the self-test.

    Mirrors the Case 195 metric schema from the issue body (annual heating
    in kWh; annual cooling in kWh; peak heating in kW). The cooling
    metric is intentionally optional so the no-drift smoke test can pass
    on identical metrics without forcing the cooling value.
    """
    directory.mkdir(parents=True, exist_ok=True)
    cooling = metric_value_cooling if metric_value_cooling is not None else metric_value
    case_payload = {
        "_doc": "synthetic",
        "case_id": "195",
        ...
        "metrics": {
            "annual_heating_kwh": metric_value,
            "annual_cooling_kwh": cooling,
            "peak_heating_kw": 1.05,
        },
    }
    (directory / "case_195_baseline.json").write_text(
...
def _populate_unpopulated_snapshot(directory: Path) -> None:
    ...
    Mirrors the placeholder state that the future implementer will
    replace: the verifier must refuse to diff this set (exit code 2).
    """
    directory.mkdir(parents=True, exist_ok=True)
    case_payload = {
        "_doc": "placeholder",
        ...
        "metrics": {
            "annual_heating_kwh": None,
            "annual_cooling_kwh": None,

...
fn main() {
    ...
    for i in 0..model.num_zones.min(3) {
        println!(
            "  Zone {}: {:.0} J/K",
            model.thermal_capacitance.as_ref()[i]
        );
    }
    println!();
    ...
    for i in 0..model.num_zones.min(3) {
        println!(
            "  Zone {}: h_tr_em = {:.2} W/K, h_tr_ms = {:.2} W/K",
            i,
            model.h_tr_em.as_ref()[i],
            model.h_tr_ms.as_ref()[i]
        );
    }
    println!();
    ...
    for i in 0..model.num_zones.min(3) {
        println!(
            "  Zone {}: h_tr_is = {:.2} W/K, h_tr_w = {:.2} W/K, h_ve = {:.2} W/K",
            i,
            model.h_tr_is.as_ref()[i],
            model.h_tr_w.as_ref()[i],
            model.h_ve.as_ref()[i]
        );
    }
    println!();

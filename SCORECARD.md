...
**Data source as of:** 2026-08-16 14:34 UTC  
...
|--------|---------|---------------|--------|--------|
| ASHRAE 140 pass rate | **13.1%** (11/84 metrics) | ≥ 60% (`validation.min_pass_rate`) | ❌ Fail | `docs/ASHRAE140_RESULTS.md` |
| Mean Absolute Error (MAE) | **52.25%** | ≤ 50% (`validation.max_mae`) | ❌ Fail | `docs/ASHRAE140_RESULTS.md` |
...
| Validation-suite throughput | 18.75 cases/sec | (informational) | ℹ️ | `docs/ASHRAE140_RESULTS.md` |
| Max single-case deviation | 470.11% | (ref: `individual.max_deviation` = 100%) | ℹ️ | `docs/ASHRAE140_RESULTS.md` |

## ASHRAE 140 Pass Rate

- **Overall (metric-level):** 13.1% — 11 PASS / 8 WARN / 65 FAIL of 84 results. Below the 60% gate.
...
- **Validation-suite throughput:** 18.75 cases/sec — informational only; this is the test-runner cadence, not the BatchOracle benchmark (source: `docs/ASHRAE140_RESULTS.md`).

## MAE vs Budget

- **Gate:** ≤ **50%** (`validation.max_mae`).
- **Current:** **52.25%** — Over budget by +2.25 pp. Max single-case deviation 470.11%.

...
Building Energy Modeling engine (Rust + Python + Node bindings). Mid-milestone on v1.3 "Blind ASHRAE 140 Validation". Canonical pass-rate figure and gate-budget comparison live in `SCORECARD.md` (auto-generated by `scripts/generate_scorecard.py`); see also `docs/ASHRAE140_RESULTS.md` and `README.md` for status context.
...
- **Local-only runtime dirs** (gitignored — never commit, never create at repo root): `.sdd/` (Bernstein agent-orchestration runtime, #2837), `.opencode/` (OpenCode plugin config), `.claude/` (skills), `.gitnexus/`, `.serena/`, `.automaker/`, `.jules/`, `.sisyphus/`, `.superset/`, `.planning/worktrees/`, and as of #2981, **`.agents/`** (orchestration telemetry — issues/, results/, skills/, coverage/ written by `github-wave-orchestrator`, `parallel-issue-generator`, `pytest`, and `bin/agent-review`). The matching `.gitignore` entries for each of these runtime dirs are the **defence** — every fresh `git clone` already accepts them on each developer's machine without them ever being committed. As of #3076, every one of these dirs has **zero entries in `git ls-files`** across the nine AGENTS-listed runtime dirs (the #2984 untrack step removed 41 legacy tracked files: 27 `.automaker/`, 5 `.serena/`, 5 `.sisyphus/`, 2 `.gitnexus/`, 1 `.jules/`, 1 `.superset/`; the #3076 follow-up removed a further 104 `.agents/` files that #2984 had missed); `.agents/` is gitignored at the dir level (#2981) so NEW result files no longer block `git pull --ff-only`. Regression is locked in by `scripts/ci/test_check_root_hygiene.py::test_agents_md_runtime_dir_has_no_tracked_files` (parametrised over the 6 dirs originally covered by #2984) and `test_agents_md_runtime_dirs_have_gitignore_entries` (belt-and-braces gitignore-presence check); `.agents/` is pinned by `test_agents_dir_has_gitignore_entry` + `test_agents_dir_not_in_root_dotfile_allowlist` + `test_main_passes_after_agents_gitignore_added`. **Defence-in-depth (issue #3076)**: `scripts/check_root_hygiene.py::find_tracked_in_gitignored_dirs` now probes `git ls-files <dir>/` for every entry in `TRACKED_BUT_IGNORED_RUNTIME_DIRS` (`.agents`, `.automaker`, `.claude`, `.gitnexus`, `.jules`, `.opencode`, `.serena`, `.sisyphus`, `.superset`) and fails CI if any tracked entry reappears — the dotfile/dotdir cross-check (#2954) only inspects the directory itself, not the files inside, so this separate probe is required to keep the regression from recurring. Worktrees mounted under `~/.superset/` and `~/.planning/worktrees/` are **always preserved** by `scripts/cleanup_stale_worktrees.sh` (#3069) — see `docs/agents/worktree-cleanup.md` for the operator guide; the script is dry-run by default and refuses to delete `main`/`develop`, the current worktree, or any branch with unpushed commits. Pinned by `test_find_tracked_in_gitignored_dirs_flags_tracked_files` (parametrised over the 9 dirs), `test_find_tracked_in_gitignored_dirs_empty_when_clean`, `test_find_tracked_in_gitignored_dirs_outside_git_returns_empty`, `test_main_fails_on_tracked_but_ignored_file_then_passes_after_untrack` (end-to-end), `test_scan_root_records_tracked_but_ignored_bucket`, and `test_tracked_but_ignored_runtime_dirs_tuple_matches_gitignore`.

...
- **MSRV pinned to 1.81** (#2934). All 11 `Cargo.toml` files (root + `fluxion-core`, `fluxion-mcp`, `fluxion-city`, `fluxion-cfd`, `fluxion-fluid`, `fluxion-wasm`, `fluxion-grid`, `fluxion-behavior`, `crates/fluxion-twin`, `crates/fluxion-toon`) declare `rust-version = "1.81"` on `[package]`, and the root additionally declares it under `[workspace.package]`. The `.github/workflows/msrv.yml` workflow enforces this with `cargo +1.81 build --workspace --locked` on every PR + push to develop/main that touches a Cargo.toml, Cargo.lock, or the workflow itself; failing the merge when a change drifts past 1.81. When bumping the MSRV, update the field on **all 11** files AND the `dtolnay/rust-toolchain` SHA pin in `msrv.yml` (the action version itself is unpinned to a toolchain — `toolchain: 1.81` in the job body is what enforces the MSRV).

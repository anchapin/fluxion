...
cargo test                                           # ⚠ ROOT CRATE ONLY (NOT the full suite) — `default-members = ["."]` makes bare `cargo test` skip the 4,158 sibling-crate tests; do NOT rely on this as a green-light signal
...
| `cargo test --workspace --exclude fluxion-tauri` | full workspace (lib + integration + bin) | 8,063 | 153 | `tests/test_inventory.json::totals.workspace_tests` / `workspace_ignored` (cargo-verified via `--verify`; the AST-regex snapshot is in `tests/reference_data/test_inventory_baseline.json::metrics_ast`) |
| AST-regex inventory | committed in `tests/reference_data/test_inventory_baseline.json::metrics_ast` | 8,760 | 138 | non-runtime snapshot, used by the drift gate (`--no-verify`); AST counts run ~10% high vs cargo-verified because the regex doesn't track `cfg(test)` boundaries |

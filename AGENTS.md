...
> **Workspace-scope rule (Issue #3587)** — The root crate is also workspace package `fluxion` with `default-members = ["."]`. The bare `cargo test` therefore runs the root crate ONLY (3,793 lib tests + the root `[[test]]` entries) and silently SKIPS the remaining 4,085 sibling-crate tests (`::totals.workspace_tests` minus `::totals.lib_tests_root`). **Always use the workspace form below** unless you have a deliberate reason to scope to one crate. See `docs/agents/workspace-scope.md` for the full rationale and `.githooks/pre-push` for the opt-in pre-push gate.
...
cargo test                                           # ⚠ ROOT CRATE ONLY (NOT the full suite) — `default-members = ["."]` makes bare `cargo test` skip the 4,085 sibling-crate tests; do NOT rely on this as a green-light signal
...
|---|---|---|---|---|
| `cargo test --lib` | root crate unit tests | 3,793 | 6 | matches `tests/test_inventory.json::totals.lib_tests_root` (cargo-verified — preferred over the AST-regex scan when both are available; see `verify.matched` in the JSON) |
| `cargo test --workspace --exclude fluxion-tauri` | full workspace (lib + integration + bin) | 7,878 | 134 | `tests/test_inventory.json::totals.workspace_tests` / `::totals.workspace_ignored` (cargo-verified via `python3 scripts/generate_test_inventory.py --verify`; regenerate when adding/removing tests so this table stays accurate) |
| AST-regex inventory | committed in `tests/test_inventory.json` | 7,878 | 134 | same JSON, cargo-verified numbers; the AST-regex scan lives in `by_crate` and the `workspace_lib_tests` / `workspace_integration_tests` split (used by the drift gate's `--no-verify` fast path) |
| Cargo test binaries | root `tests/*.rs` keepers + consolidated `all_tests` runner + hand-wired `[[test]] path = "tests/<sub>/<foo>.rs"` + sibling-crate targets | 47 | n/a | matches `tests/test_inventory.json::totals.test_binaries` (273 standalone root binaries consolidated into `all_tests` + 10 keepers in Issue #3764; `BASELINE_TEST_BINARIES = 309` in `scripts/check_test_inventory_drift.py` is the highest-ever-accepted ratchet and still satisfies `live < ratchet`) |

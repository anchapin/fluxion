...
**ONNX surrogate** — `FLUXION_ONNX_MODEL` (default `models/surrogate_zone_thermal.onnx`; mock fallback when unset), `FLUXION_ONNX_BACKEND` (`cpu|cuda|coreml|directml|openvino`; auto-downgrades to `cpu` if `cuda` feature not built), `FLUXION_GPU=0` to force CPU inference. **SHA-256 integrity check (#2906):** every load goes through `verify_onnx_signature` (`src/ai/surrogate.rs`) which reads `<model>.sha256`, parses it as `sha256sum` output, computes the model's actual SHA-256, and **refuses to load on mismatch (fail-closed)**. Set `FLUXION_ONNX_MODEL_SIGNATURE=<64-hex-digest>` to override the manifest for rotated models (e.g. CI swap tests) until the manifest is updated. CI gate: `.github/workflows/onnx-integrity.yml` re-verifies the manifest format and runs the verify-path unit tests on every PR that touches the model, manifest, load code, or the gate itself.
...
- **MSRV pinned to 1.89** (#2934). All 11 `Cargo.toml` files (root + `fluxion-core`, `fluxion-mcp`, `fluxion-city`, `fluxion-cfd`, `fluxion-fluid`, `fluxion-wasm`, `fluxion-grid`, `fluxion-behavior`, `crates/fluxion-twin`, `crates/fluxion-toon`) declare `rust-version = "1.89"` on `[package]`, and the root additionally declares it under `[workspace.package]`. The `.github/workflows/msrv.yml` workflow enforces this with `cargo +1.89 build --workspace --locked` on every PR + push to develop/main that touches a Cargo.toml, Cargo.lock, or the workflow itself; failing the merge when a change drifts past 1.89. (Originally targeted 1.81, but clap_lex 1.1.0 uses edition2024 (needs Cargo 1.85+), and nalgebra 0.35 / safe_arch 1.1 / statrs 0.19.1 / wide 1.6.1 each declare rustc >= 1.89; 1.89 is the lowest stable that builds today per the issue's "or the lowest stable that builds today" clause.) When bumping the MSRV, update the field on **all 11** files AND the `dtolnay/rust-toolchain` SHA pin + `toolchain:` field in `msrv.yml`.
...
- **Local-only runtime dirs** (gitignored — never commit, never create at repo root): `.sdd/` (Bernstein agent-orchestration runtime, #2837), `.opencode/` (OpenCode plugin config), `.claude/` (skills), `.gitnexus/`, `.serena/`, `.automaker/`, `.jules/`, `.sisyphus/`, `.superset/`, `.planning/worktrees/`, and as of #2981, **`.agents/`** (orchestration telemetry — issues/, results/, skills/, coverage/ written by `github-wave-orchestrator`, `parallel-issue-generator`, `pytest`, and `bin/agent-review`). The matching `.gitignore` entries for each of these runtime dirs are the **defence** — every fresh `git clone` already accepts them on each developer's machine without them ever being committed. As of #2984, every one of these dirs has **zero entries in `git ls-files`** for the six AGENTS-listed runtime dirs (the untrack step removed 41 legacy tracked files: 27 `.automaker/`, 5 `.serena/`, 5 `.sisyphus/`, 2 `.gitnexus/`, 1 `.jules/`, 1 `.superset/`); `.agents/` is gitignored at the dir level (#2981) so NEW result files no longer block `git pull --ff-only`. Regression is locked in by `scripts/ci/test_check_root_hygiene.py::test_agents_md_runtime_dir_has_no_tracked_files` (parametrised over the 6 dirs) and `test_agents_md_runtime_dirs_have_gitignore_entries` (belt-and-braces gitignore-presence check); `.agents/` is pinned by `test_agents_dir_has_gitignore_entry` + `test_agents_dir_not_in_root_dotfile_allowlist` + `test_main_passes_after_agents_gitignore_added`.
- **Dotfile / dotdir cross-check** (#2954): `scripts/check_root_hygiene.py` also scans every dotfile/dotdir at the repo root and reports it as a violation when it is NOT in `ROOT_DOTFILE_ALLOWLIST` (legit root config: `.git/`, `.gitignore`, `.cargo/`, `.github/`, ...), NOT matched by `.gitignore`, AND NOT already tracked in git. If the gate flags a runtime dir you genuinely need, add the matching `.gitignore` line (preferred — keeps it out of every future commit) OR add the dir to `ROOT_DOTFILE_ALLOWLIST` (only when the dir is legit root config that must stay tracked). The previous gate silently skipped all dotfile-prefixed entries, so any agent-runtime dir that landed at root before this gate existed was structurally invisible to CI.

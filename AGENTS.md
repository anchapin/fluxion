# Agent's `gh` CLI Usage Notes & Development Guidelines

## Development Philosophy: Tests Define the Goal

**CRITICAL PRINCIPLE:** Tests define where the code should be. When tests fail:
1. **Default Action:** Change ONLY the referenced implementation code to make the test pass
2. **Exception:** Only modify the test itself if there is a clear bug or typo IN the test
3. **Never:** Change tests to match incorrect implementation behavior

This ensures tests remain the source of truth for expected behavior and prevents test drift from actual requirements.

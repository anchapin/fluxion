...
- **Local-only runtime dirs** (gitignored — never commit, never create at repo root): `.sdd/` (Bernstein agent-orchestration runtime, #2837), `.opencode/` (OpenCode plugin config), `.claude/` (skills), `.gitnexus/`, `.serena/`, `.automaker/`, `.jules/`, `.sisyphus/`, `.superset/`, `.planning/worktrees/`. `CLAUDE.md` is auto-generated per-session and `.gitignore`d. The matching `.gitignore` entries for each of these runtime dirs are the **defence** — every fresh `git clone` already accepts them on each developer's machine without them ever being committed. As of #2984, every one of these dirs has **zero entries in `git ls-files`** (the untrack step removed 41 legacy tracked files: 27 `.automaker/`, 5 `.serena/`, 5 `.sisyphus/`, 2 `.gitnexus/`, 1 `.jules/`, 1 `.superset/`). Regression is locked in by `scripts/ci/test_check_root_hygiene.py::test_agents_md_runtime_dir_has_no_tracked_files` (parametrised over the 6 dirs) and `test_agents_md_runtime_dirs_have_gitignore_entries` (belt-and-braces gitignore-presence check).

...
- `Multi-Zone Perf Gate (Issue #2772)` — PR-blocking ABSOLUTE multi-zone floor: ≥ 10 cfg/s on a 10-zone model (pop_1000, analytical mode; `release_gates.yaml` → `benchmark.multi_zone.min_configs_per_sec`). Runs the new `test_multi_zone_performance_regression` ×3 → median → `release_gate_checker.py --benchmark-gates multi_zone_throughput` in `performance_dashboard.yml`. The #2693 gate measures the SINGLE-ZONE analytical path (150 cfg/s) — a much easier target — so a per-zone-cost regression could pass #2693 and blow the multi-zone margin (only 2.8× per `benches/multi_zone_throughput.rs:17`); this gate closes that hole. Same runner / median-of-3 discipline as #2693.
- `Architecture Drift Detection` (nightly + on `src/**/*.rs`/`ARCHITECTURE.md` changes)
- `Docs Hygiene Gate` (#2466; freshness sub-check #2765) · `Code Coverage Gate` (#1932, ratchet; baseline in `validation/coverage_baseline.json`)
...
- **`docs/**/*.md` must have a 7-line summary at the top (lines 2–8)**. CI gate: `scripts/check_docs_summaries.py` (#2466). AGENTS.md is exempt. After adding/removing files under `docs/`, run `scripts/generate_doc_inventory.py` to refresh the auto-table in `docs/doc-inventory.md`. Drift is blocked by `scripts/check_doc_inventory_fresh.py` (#2765) — a content-based gate that runs the generator and diff-compares; if it fails locally or in CI, run the generator and commit the regenerated `docs/doc-inventory.md`.
...
| `scripts/check_{root_md_policy,docs_summaries,doc_inventory_fresh}.py` · `scripts/generate_doc_inventory.py` | Docs-hygiene gates (#2466 + #2765 freshness) |

...
//!
//! This module provides the **default physics-based implementation** of the
//! [`ThermalModelTrait`] from `crate::sim::thermal_model`.
//!
//! # Purpose
//! - This is the **physics adapter**: analytical 5R1C thermal network calculations
//! - Use [`ThermalModel`] when you need the default physics-based thermal model
//! - For trait-based polymorphic usage, use [`ThermalModelTrait`] from `crate::sim::thermal_model`
//!
//! # Architecture
//! ```
//!                    ┌─────────────────────────┐
//!                    │   ThermalModelTrait      │  ← Public interface (sim/thermal_model.rs)
//!                    │   (trait)                │
//!                    └────────────┬────────────┘
//!                               implements
//!                    ┌────────────┴────────────┐
//!                    │                          │
//!              ┌─────┴─────┐            ┌──────┴──────┐
//!              │ Physics   │            │ Surrogate   │  ← Adapter implementations
...
//!              │                                      │
//!              ▼                                      ▼
//!     thermal/physics_adapter.rs          (neural network wrapper)
//!     (THIS FILE - default physics impl)
//! ```
//!
//! # Usage
//! ```
//! use fluxion::thermal::physics_adapter::ThermalModel;
//! use fluxion::sim::thermal_model::ThermalModelTrait;
//!
//! // Direct use of physics adapter
//! let model = ThermalModel::new(3, 20.0);
//!
//! // Trait-based polymorphic use
//! fn use_model(model: &dyn ThermalModelTrait) { ... }
//! ```

...
2026-09-18 −1 for #3871's leaf-module hoist of `ZoneCountPolicy` +
`MAX_ZONES` into `fluxion-core::zone_count_policy` that re-closes the
residual sim→physics edge admitted by #3869 at 2026-09-18);
the script exits
...
**Key struct**: `ThermalManifold` in `physics/geometry_tensor/manifold.rs`
...
**Zone-count envelope policy (Issue #3731, Phase A8 follow-on):** the `MAX_ZONES = 100` constant from test (7) is the hard envelope of the Phase-1a data structures (`(MAX_ZONES, 20)`-shaped geometry tensors, `(MAX_ZONES, MAX_ZONES)` adjacency, `(MAX_ZONES, 5)` per-zone properties, and the gauge dispatcher's per-zone internal buffers). Once the `gauge-solver` cargo feature flips unconditionally at the §LIMIT-21 β-soak closure (Issue #3286), a spec with `num_zones > MAX_ZONES` routed through `ZoneSolverKind::Gauge` (the unconditional `ThermalSelector::default()` per AGENTS.md Phase A8 note) would otherwise reach the deep `assert!` in the shadow-mode `ThermalManifold::new(num_zones)` (`src/physics/gauge_solver.rs:28`) and panic instead of surfacing the typed `PhysicsError::initialization` every other gauge pre-check uses. The canonical typed seam is `physics::geometry_tensor::manifold::ZoneCountPolicy` (`src/physics/geometry_tensor/manifold.rs`):

- `ZoneCountTier::Empty` — `num_zones == 0`: programming error from `from_spec`; typed rejection here surfaces the bug at the entry point.
- `ZoneCountTier::Standard` — `1 <= num_zones < MAX_ZONES`: happy path; gauge dispatch may proceed.
- `ZoneCountTier::AtCapacity` — `num_zones == MAX_ZONES` (== 100): permitted but flagged for dashboards (last-slot use).
- `ZoneCountTier::BeyondEnvelope` — `num_zones > MAX_ZONES`: typed rejection via `PhysicsError::initialization` naming the offending count, the gauge envelope constant, and the §LIMIT-21 / Phase-1b geometry rework that owns growing the envelope.

`ZoneCountPolicy::check_gauge()` is wired into **both** gauge initialization entry points — `enable_gauge_solver(spec)?` (single-zone, `src/sim/thermal_model_core/mod.rs:2952`) and `enable_gauge_solver_multi_zone(spec)?` (multi-zone, `src/sim/thermal_model_core/mod.rs:3164`) — **outside** the `#[cfg(feature = "gauge-solver")]` gate, so the rejection surface is identical in both feature states (default build and `--features gauge-solver`). Callers wanting a non-reject action (explicit `NineRFourC` fallback, partitioned gauge solve, etc.) must invoke `ZoneCountPolicy::for_count(n)` and switch on the tier; bypassing the wrapper is forbidden because the deep `ThermalManifold::new(num_zones)` panic remains as the secondary, deep-defense check and SHOULD be unreachable from the public gauge API. The boundary tests (`tests_zone_count_policy_*` in `src/physics/geometry_tensor/manifold.rs::tests`, `enable_gauge_solver_*_rejects_beyond_envelope_*` in `src/sim/thermal_model_core/tests.rs`) pin the typed behavior for `Empty`, `Standard`, `AtCapacity` (= 100), `BeyondEnvelope` (= 101), and a far-beyond case (200) across both feature states.
...
### Gated files (live ceilings)

Snapshot 2026-09-13; run `python3 scripts/check_module_size.py --json` for current numbers.
...
| `src/physics/geometry_tensor/` (dir — `mod.rs` 1440, `manifold.rs` 676, `geometry.rs` 224, `qubo.rs` 125) | ~2465 total | ~2589 | #3574, #3870 |
| `src/validation/reporter.rs` | 2183 | 2292 | #3574 |
| `fluxion-city/src/lib.rs` | 2680 | 2814 | #3574 |
| `fluxion-mcp/src/tools.rs` | 1648 | 1748 | #3574 |
...
- **Decomposition** — split a gated file: remove its `LIMITS` entry AND its line in the `_BASELINE_MODULE_SIZE_LIMITS_SET` freeze snapshot, delete the stale ratchet JSON under `tests/reference_data/module_size/`, and lower `BASELINE_MODULE_SIZE_LIMITS` by one. The decomposition itself is the ratchet-lowering event; no child entries are added (precedents: Issues #3543, #3625, #3669).
- **New gated entry** — requires raising `BASELINE_MODULE_SIZE_LIMITS` with a documenting comment naming the tracking issue, adding the path to `_BASELINE_MODULE_SIZE_LIMITS_SET`, and appending the `LIMITS` entry — all in the same PR. The baseline is otherwise downward-only.

...
ALL of `src/sim/**/*.rs` — an 81-edge sim→physics baseline
(`BASELINE_SIM_TO_PHYSICS = 81`; 84 pre-existing `use crate::physics::`
...
the new baseline. The documented baseline is now **0+81 edges** (0
physics→sim + 81 sim→physics) after the subsequent companion-cycle-work

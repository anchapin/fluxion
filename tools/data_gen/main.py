...
from tools.data_gen import weather, geometry, simulation
...
def main():
    ...
    weather_parser = subparsers.add_parser("download-weather", help="Download standard weather files")
    weather_parser.add_argument("--out", default="assets/weather", help="Output directory for weather files")

    # Subcommand: generate
    gen_parser = subparsers.add_parser("generate", help="Generate synthetic data (Geometry + Simulation)")
    gen_parser.add_argument("--out", default="assets/synthetic_data", help="Output directory")
    gen_parser.add_argument("--weather-dir", default="assets/weather", help="Directory containing weather files")
    gen_parser.add_argument("--weather-file", default=None, help="Specific weather file name (optional)")
    gen_parser.add_argument("--count", type=int, default=1, help="Number of simulations to run")
    ...
    if args.command == "download-weather":
        ...
        if not os.path.exists(args.weather_dir):
            logger.info(f"Weather directory {args.weather_dir} not found. Downloading defaults...")
        ...
        if args.weather_file:
            epw_path = weather.get_weather_file_path(args.weather_dir, args.weather_file)
        else:
        ...
        for i in range(args.count):
            ...
            l = random.uniform(args.length_min, args.length_max)
            wwr = random.uniform(args.wwr_min, args.wwr_max)
            orient = random.uniform(0, 360)

            logger.info(f"  Params: {w=:.2f}, {l=:.2f}, {wwr=:.2f}, {orient=:.1f}")

            # Create Model
            model = geometry.create_shoebox_model(
                width=w, length=l, height=args.height,
                wwr=wwr, orientation_degrees=orient
            )

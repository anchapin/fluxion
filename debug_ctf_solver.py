...
def extract_ctf_debug_info():
    ...
    ctf_fix = re.findall(
        r'SESSION 48 FIX: CTF flux to zone air: Q_CTF=([\d.-]+) W',
        output
    )

    # Extract CTF enablement info
    ctf_enabled = re.findall(r'SESSION 48: CTF solver (ACTIVE|INACTIVE|ENABLED)', output)
    ...
    if x_coeffs and y_coeffs:
        ...
        if 'u_value' in locals():
            if abs(x0 - actual_u) < 0.1:
                print(f"✅ X[0] ≈ U-value (correct)")
            else:
                print(f"❌ X[0] should ≈ U-value")

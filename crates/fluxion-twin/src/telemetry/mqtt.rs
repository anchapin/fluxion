...
//! rejected** — TLS is required. There is **no runtime bypass** for certificate
//! verification (Issue #3162).
//!
//! # Example
//!
//! ```ignore
...
impl MqttTelemetryConsumer {
    ...
    /// required and there is no runtime bypass for certificate verification
    /// (Issue #3162).
    ///
    /// # Errors
    ///
    ...
    match scheme {
        BrokerScheme::Plaintext => Err(MqttTelemetryError::InvalidConfig(
            "plaintext broker URL ('mqtt://'/'tcp://') rejected: telemetry would be \
             unencrypted. TLS (mqtts://, port 8883) is required.".to_string(),
        )),
        BrokerScheme::Tls => Ok(()),
    }
}

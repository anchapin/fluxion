...
pub struct EspRCliResult {
    ...
    pub pass_rate: f64,
    /// Validation report
    pub report: crate::validation::reports::CrossValidationReport,
    /// Execution timestamp
    pub timestamp: u64,
...
pub fn run_cli_validation(config: &EspRCliConfig) -> Result<EspRCliResult, Box<dyn Error>> {
    ...
    if let Some(output_path) = &config.output_path {
        save_report(
            &config,
            &EspRCliResult::new(passed, pass_rate, report, None),
            output_path,
        )?;
    }

    Ok(EspRCliResult::new(passed, pass_rate, report, None))
}
...
/// Parse Fluxion configuration from JSON file
fn parse_fluxion_config(
    path: &Path,
) -> Result<crate::simulation::SimulationConfig, Box<dyn Error>> {
    let file_content = std::fs::read_to_string(path)?;
    let config: crate::simulation::SimulationConfig = serde_json::from_str(&file_content)?;
    Ok(config)
}

/// Run Fluxion simulation with given configuration
fn run_fluxion_simulation(
    ...
    let mut simulation = crate::simulation::Simulation::from_config(config.clone());

    // Run simulation
    simulation.run()?;

    // Extract validation results
    let results = simulation.generate_validation_results();
...
fn generate_markdown_report(result: &EspRCliResult) -> Result<String, Box<dyn Error>> {

...
use crate::validation::reports::cross_validation::CrossValidationReport;
...
pub fn advanced_cross_validation_example() -> Result<(), Box<dyn Error>> {
    ...
    for (zone_name, zone_result) in &report.zone_results {
        println!(
            "  Zone '{}': pass={}, avg_diff={:.2}°C, max_diff={:.2}°C",
            zone_name, zone_result.pass, zone_result.average_difference, zone_result.max_difference
        );
    }
...
pub fn report_generation_example() -> Result<(), Box<dyn Error>> {
    ...
    for (zone_name, zone_result) in &report.zone_results {
        markdown_report.push_str(&format!(
            "- **{}**: Pass={}, Avg Diff={:.2}°C, Max Diff={:.2}°C\n",
            zone_name, zone_result.pass, zone_result.average_difference, zone_result.max_difference
        ));
    }

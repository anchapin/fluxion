...
def validate_flux_integration(results: Dict) -> bool:
    ...
    print(f"{Colors.GREEN}✅ No old buggy version detected (no Q_net in output){Colors.NC}")
    ...
    if fluxes:
        ...
        if len(fluxes) >= 2:
            ratio = max(fluxes[0], fluxes[1]) / min(fluxes[0], fluxes[1]) if min(fluxes[0], fluxes[1]) != 0 else 0
            if ratio > 10.0:
                print(f"{Colors.YELLOW}      ⚠️  WARN: Large flux ratio detected ({ratio:.2f}x){Colors.NC}")
...
def main():
    ...
    peak_pass = validate_peak_loads(results)
    ...
    print(f"   Peak Loads: ✅ PASS (warnings only)")

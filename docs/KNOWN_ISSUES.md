...
### LIMIT-07: Default-schema `/v1/simulate` diverges at timestep 91 (un-initialized model physics)

- **Description:** `run_simulation` (`src/api/server.rs`) and the `/v1/simulate/stream` handler construct the engine model via `ThermalModel::<VectorField>::new(num_zones)` and set only the heating/cooling setpoints — they never transfer the schema's geometry/construction/weather into the model's physical parameters. `ThermalModel::new()` initialises `thermal_capacitance` to a `1.0 J/K` placeholder and `air_thermal_capacitance` to `0.0` (values intended to be overwritten by `ThermalModel::from_spec`, which the REST/schema path never calls). With `C_m = 1.0 J/K`, `select_integration_method` (`src/sim/thermal_integration.rs`) selects the conditionally-stable **Explicit-Euler** mass integrator (`C_m <= 500` threshold). The Explicit-Euler update `Tm_new = Tm_old + (q_net / C_m) · dt` with `C_m = 1.0` and `dt = 3600 s` multiplies every watt of net mass flux by ~3600, producing an exponential blow-up (sign-alternating, magnitude ×~10⁶/step) that reaches `inf`/`NaN` at hourly index **91** (`last_known_good_timestep = 90`). The divergence guard at `src/api/server.rs:1191` then surfaces `SimulationFailed("simulation diverged at timestep 91 in zone zone_0")`, returning HTTP 500. This matches the warning left in `src/sim/thermal_model_physics/solver_core.rs` (warm-up block): *"ThermalModel::new() doesn't properly initialize physics parameters (cm=1.0 instead of real values), causing numerical explosion during warmup."*

- **Affected Paths:** REST `/v1/simulate`, `/v1/simulate/stream`, `/v1/batch/simulate` — any caller driving `run_simulation` over a default `SimulationSchemaV1`. NOT the ASHRAE 140 validation path (which uses `from_spec`) and NOT the Python bindings `simulate_multi_zone` (user-constructed models).

- **Affected Tests:** 6 pre-existing CI failures — `simulate_returns_heating_and_cooling_energy`, `simulate_matches_in_process_within_tolerance`, `simulate_peak_loads_match_in_process`, `batch_simulate_returns_results_for_multiple_schemas`, `batch_simulate_partial_failure_contains_error_strings` (`tests/api_integration_tests.rs`) and `concurrent_throughput_smoke` (`tests/api_concurrent_throughput.rs`).

- **Severity:** High (permanently red API integration gate on every PR; erodes confidence in the `/v1/simulate` 200-path).

- **Status:** 🟡 **Isolated & tracked** (2026-08-12). The 6 tests above are `#[ignore]`'d with an explicit `#2674` reason pointer. `tests/issue_2674_repro.rs` actively pins the divergence signature (`failing_timestep == 91`, `last_known_good_timestep == 90`, trace explosion magnitude > 1e10 by step 90) so the bug is tracked, not hidden — when the real fix lands that test fails, which is the signal to remove the `#[ignore]`s.

- **Resolution Notes / why not fixed yet:** The correct fix is to initialise the full physics field set from the schema (an `from_spec`-equivalent for `SimulationSchemaV1`): air-node and mass-node capacitances from `ρ·cp·V` and the construction layers, plus the envelope conductances (`h_tr_em`, `derived_h_tr_3`, …) from the layer k-values/areas. This is ~250 lines of new schema→physics mapping plus EnergyPlus-comparable validation to trust the resulting EUI — not a safe one-pass change. A partial fix (capacitances only, which lifts `C_m` above the Crank-Nicolson threshold and stops the NaN) was prototyped and **rejected**: with the envelope conductances still at their `0.0` placeholders the mass node decouples from the zone air and the simulation now returns *finite but physically nonsensical* results (zone temperatures swinging to ±5×10⁵ °C, EUI ≈ −4.4×10⁴ kWh/m²/yr). Silent garbage through the REST API is strictly worse than a clean, attributed 500, so the divergence is left intact and tracked here pending the full wiring.

- **Potential Solutions:**
  1. **(Preferred)** Implement `ThermalModel::from_schema(&SimulationSchemaV1)` mirroring the construction-layer → `h_tr_em` / `C_m` logic in `from_spec`, and call it from `run_simulation` + `simulate_stream` after `ThermalModel::new`. Validate against an EnergyPlus comparable for the default fixture before un-ignoring.
  2. Defence-in-depth in `select_integration_method`: never select Explicit-Euler when the stability criterion `dt < C_m / (h_tr_em + h_tr_ms)` is violated (it is essentially never satisfied at `dt = 3600 s`); fall back to the unconditionally-stable Crank-Nicolson. This would make `ThermalModel::new()` robust to forgotten initialisation in any caller, not just the API path. (Does not by itself produce correct results — still needs (1) for physical accuracy.)
  3. Wire `update_derived_parameters` to also compute `derived_h_tr_3` (currently only `from_spec` does), so the mass node is always coupled to the zone air.

- **Discovered:** Issue #2674 (wave orchestration 2026-08-10/11 — independently confirmed by ≥4 sub-agents as the identical 5-failure set on clean `develop`).

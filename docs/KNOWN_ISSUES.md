...
| **940** | Annual heating | 5.158 MWh CTF / 1.29 MWh blind vs ref [0.79, 1.41] MWh (per §LIMIT-05 UPDATE #2452, post-PR #3042); CTF path overshoots blind path 4–6× | #2870 (closed via PR #3042) | **#3062** | Wave 17 (PR #3042) |
...
| #3961 | Case 600FF zone-solver solar coupling: can the `phi_st_window` heuristic (`remaining_sol × st_sol_frac`) and the fixed 0.30/0.30 split be retired to the referee convention (to_air 0.0 / beam_to_mass 1.0) by defaults alone? | 🟡 **Refutation + spec pins shipped** — measured NO: `to_air=0` alone moves 600FF peak T_air away from the band (47.29 → 46.01 °C vs 64.9–75.1 °C) and regresses Case 600/800 cooling ~20 pp each; `beam_to_mass=1.0` collapses high-mass cooling (960: 0.144 → 0.008 MWh); upstream window-area × incident-irradiance delivery re-verified exact (8,437 W peak); the 0.30 low-mass air fraction is load-bearing compensation for missing air-node capacitance / beam-incident absorption geometry (#1152-class structural work); zero production-physics change shipped — in-code experiment record (`thermal_model_core::from_spec_with_selector`), load-bearing defaults pin + gauge telemetry-closure and legacy-partition mechanism pins in `tests/all_tests/solar_split_convention.rs`; failing `solar_distribution_validation` referee remains the tracking gate for the structural closure | §LIMIT-30 UPDATE (#3961) |
...
- `docs/investigations/issue-3979-ctf-zone-coupling.md` — Root-cause analysis
  for the LIMIT-31 CTF↔zone additive-correction double count and the
  conditioned high-mass backend bypass.

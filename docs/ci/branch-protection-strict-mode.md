...
**For `main` branch:** Use `required_checks` (14 checks; the full Lane 1 + Lane 2 union). Code-changing PRs that touch the physics path class must pass all Lane-1 + Lane-2 gates. Lane 3 is not required per-PR on `main` either — the wave orchestrator's merge freeze covers both branches.

**For `develop` branch:** Use `required_checks_workflow_only` (9 checks; Lane 1 + non-path-filtered Lane 2 items). Workflow-only PRs (docs, CI, scripts) can merge without triggering the 5 path-filtered Lane-2 items that structurally cannot run for them, and without waiting on the five Issue #3898 `paths-ignore` gates that skip docs-only diffs by design. The Lane-3 merge freeze is the develop-side authority for the 8 nightly-authority gates. Update 2026-09-25: the Tauri CSP, FFI Feature, Audit Ignore Freshness, and fluxion-grid Integration gates were demoted from required to advisory during the physics-core focus; they still run on every PR via `workflow_index` but no longer block merging.

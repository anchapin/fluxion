...
> **TL;DR**: Archived hand-written test catalog superseded by `tests/test_inventory.json` (Issue #3442).
> **Key decisions**: Pointer to JSON source-of-truth | Workspace-scope command only | No `cargo test --all`
> **Owned by**: QA / Test Infrastructure
> **Reviewed**: 2026-09-17
> **Supersedes**: `docs/test-inventory.md` (pre-Issue #3442 catalog, last reviewed 2026-07-13)

This document was the pre-Issue #3442 hand-written catalog of test files in
the Fluxion repository. It was retired by Issue #3737 because it taught
`cargo test --all` and `cargo test --test '*_isolation'`, neither of which
matches the repo-mandated suite (see `AGENTS.md` §"Commands That Are Easy
to Guess Wrong"). Bare `cargo test` runs the root crate only
(`default-members = ["."]`), and `cargo test --all` walks readers into the
known workspace-scope and tauri-build traps documented at Issue #3587.

## Canonical test inventory

The source of truth is the auto-generated cargo-verified JSON:

- [`tests/test_inventory.json`](../../tests/test_inventory.json) — counts
  per crate (`lib_tests`, `lib_ignored`, `test_binaries`) plus workspace
  totals (`workspace_tests`, `workspace_lib_tests`, `workspace_ignored`).
  Regenerated by `python3 scripts/generate_test_inventory.py --verify`
  (Issue #3442 acceptance criterion #1).

The drift gate `scripts/check_test_inventory_drift.py` (wired into
`scripts-tests.yml`) enforces downward-only `BASELINE_*` ratchets on those
counts, so a PR that grows the test suite above the documented baseline
without bumping `tests/reference_data/test_inventory_baseline.json` is
rejected.

## Repo-mandated test execution

Per `AGENTS.md` §"Commands That Are Easy to Guess Wrong" (Issue #3587),
the developer-facing default is:

```bash
cargo test --workspace --exclude fluxion-tauri --no-fail-fast
```

CI runs the nextest equivalent:

```bash
cargo nextest run --workspace --all-targets --test-threads=2 --no-fail-fast
```

`fluxion-tauri` is excluded because its proc-macro build needs
`npm run build` in `fluxion-tauri/frontend/` to materialise
`../frontend/dist` (Issue #3126).

## Why this file was archived

The original `docs/test-inventory.md` (moved here by
`git mv docs/test-inventory.md docs/archive/test-inventory.md`) was a
hand-maintained table of every `tests/*.rs` file with prose descriptions
of what each one covered. After Issue #3442 the JSON inventory replaced
both the count source-of-truth and the drift-detection mechanism; the
markdown table duplicated information that the JSON now provides
machine-verifiably, and its "Test Execution" section recommended
`cargo test --all`, which is explicitly unsafe on this workspace
(`default-members = ["."]` makes bare `cargo test` skip the sibling
crates).

See [Issue #3737](https://github.com/anchapin/fluxion/issues/3737) for
the full audit and acceptance criteria.

## Historical content (unchanged)

The hand-written catalog content from `docs/test-inventory.md` as it stood
on 2026-09-17 (pre-archive) is preserved below for traceability.

<!-- The text below was copied verbatim from docs/test-inventory.md
     immediately before the archive move; do not edit it. -->

---

# Fluxion Test Inventory (Legacy Catalog)
...
> legacy commands copied verbatim from the pre-archive `docs/test-inventory.md`.
> They are **not** the repo-mandated suite. See the "Repo-mandated test
> execution" section near the top of this file for the canonical commands.

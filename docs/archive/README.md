...
- `test-inventory.md` — Pre-Issue #3442 hand-written test catalog,
  retired by Issue #3737. The canonical test inventory is the
  cargo-verified `tests/test_inventory.json` (regenerated by
  `scripts/generate_test_inventory.py --verify`); the repo-mandated
  suite command is
  `cargo test --workspace --exclude fluxion-tauri --no-fail-fast`
  (see `AGENTS.md` §"Commands That Are Easy to Guess Wrong"). The
  legacy table is preserved here verbatim for traceability.

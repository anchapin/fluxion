...
A bare `cargo test` therefore runs the root crate ONLY (3,835 lib tests per `tests/test_inventory.json::totals.lib_tests_root` + the explicit `[[test]]` entries) and silently SKIPS the remaining 4,092 sibling-crate tests (`::totals.workspace_tests` minus `::totals.lib_tests_root`).

...
fn test_mass_energy_change_accumulates_over_timesteps() {
    ...
    println!(
        "Mass energy change cumulative: {} J",
        final_cumulative
    );
...
fn test_cloning_preserves_thermal_mass_fields() {
    ...
    assert_eq!(
        cloned.thermal_mass_energy_accounting,
        model.thermal_mass_energy_accounting,
    ...
    assert!(
        (cloned.mass_energy_change_cumulative - model.mass_energy_change_cumulative).abs()
            < 0.001,
...
fn test_thermal_mass_correction_factor_from_case_spec() {
    ...
    println!("Case 600 correction factor: {}", model_600.thermal_mass_correction_factor);
    println!("Case 900 correction factor: {}", model_900.thermal_mass_correction_factor);
...
fn test_thermal_mass_accounting_different_outdoor_temps() {
    ...
    for t in 0..24 {
        ...
        assert!(energy.is_finite(), "Energy should be finite at constant outdoor temp");
    }
...
fn test_thermal_mass_fields_in_multi_zone_model() {
    ...
    for t in 0..10 {
        let energy = model.step_physics(t, 20.0);
        assert!(energy.is_finite(), "Energy should be finite for multi-zone model");
    }
...
fn test_thermal_mass_correction_affects_hvac_output() {
    ...
    println!("Default correction (1.0) total energy: {} kWh", energy_default);
    println!("Reduced correction (0.5) total energy: {} kWh", energy_corrected);
...
fn test_energy_accounting_comparison_enabled_vs_disabled() {
    ...
    println!(
        "Energy with accounting enabled: {} kWh",
        energy_enabled
    );
    println!(
        "Energy with accounting disabled: {} kWh",
        energy_disabled
    );
    println!(

...
fn solar_fields() {
    ...
    for step in 0..8760 {
        ...
        if step >= day_start && step < day_start + 24 {
            ...
            let sol = model.solar.solar_gains.as_slice()[0]; // W/m² of floor
            let opsol = model.solar.opaque_solar_gains.as_slice()[0];
            let area = model.setpoints.zone_area.as_slice()[0]; // m²
            let sol_w = sol * area; // total W
            let opsol_w = opsol * area;
            let phi_m = sol_w * model.solar.solar_beam_to_mass_fraction + opsol_w;
            println!(

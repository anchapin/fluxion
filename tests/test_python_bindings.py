...
class TestVectorFieldNumpyConversion:
    ...
    def test_numpy_roundtrip(self, fluxion_module):
        ...
        assert all(
            abs(a - b) < 1e-10 for a, b in zip(recovered_data, original_data.tolist())
        )

    def test_empty_vector_field_numpy(self, fluxion_module):
        vf = fluxion_module.VectorField([])
        assert vf.len() == 0

    def test_large_vector_field(self, fluxion_module):
        data = list(range(1000))
        vf = fluxion_module.VectorField(data)
        assert vf.len() == 1000
        result = vf.integrate()
        expected = sum(data)
...
class TestWallSurface:
    def test_create_wall_surface(self, fluxion_module):
        wall = fluxion_module.WallSurface(area=50.0, u_value=2.5, orientation="south")
...
class TestBatchOracle:
    ...
    def test_evaluate_multiple_candidates(self, batch_oracle):
        population = [[1.5, 20.0, 27.0], [2.0, 21.0, 28.0], [1.0, 22.0, 29.0]]
        results = batch_oracle.evaluate_population(population, use_surrogates=False)
...
class TestParameterValidation:
    def test_invalid_u_value_negative(self, batch_oracle):
        ...
        results = batch_oracle.evaluate_population(population, use_surrogates=False)
        assert results[0] != results[0]  # NaN check
    ...
    def test_valid_boundary_values(self, batch_oracle):
        population = [
            [0.1, 15.0, 22.0],  # Minimum valid values
            [5.0, 25.0, 32.0],  # Maximum valid values
            [1.5, 20.0, 27.0],  # Mid-range values
        ]
        results = batch_oracle.evaluate_population(population, use_surrogates=False)
    ...
    def test_mixed_valid_invalid(self, batch_oracle):
        population = [
            [1.5, 20.0, 27.0],  # Valid
            [-1.0, 20.0, 27.0],  # Invalid (negative U-value)
            [1.5, 500.0, 27.0],  # Invalid (heating too high)
            [1.5, 20.0, 27.0],  # Valid
        ]
        results = batch_oracle.evaluate_population(population, use_surrogates=False)

        assert len(results) == 4
        assert results[0] >= 0.0  # Valid
        assert results[1] != results[1]  # NaN (Invalid)
        assert results[2] != results[2]  # NaN (Invalid)
        assert results[3] >= 0.0  # Valid

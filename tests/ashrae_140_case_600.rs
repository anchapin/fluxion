...
fn test_case_600_baseline_ashrae_140_reference() {
    ...
    assert!(result.annual_heating_mwh + result.annual_cooling_mwh > 0.0,
    ...
    assert!(total_energy > 0.01, "Total energy {} MWh should be non-trivial", total_energy);
    ...
    assert!(min_temp < 30.0, "Minimum temperature {}°C should be below 30°C", min_temp);
    assert!(max_temp > 15.0, "Maximum temperature {}°C should be above 15°C", max_temp);

    // Verify positive energy values
    assert!(result.annual_heating_mwh > 0.0, "Annual heating should be positive");
    assert!(result.annual_cooling_mwh > 0.0, "Annual cooling should be positive");
    assert!(result.peak_heating_kw > 0.0, "Peak heating should be positive");
    assert!(result.peak_cooling_kw > 0.0, "Peak cooling should be positive");
}
...
fn test_case_600_model_creation() {
    ...
    assert_eq!(model.model.heating_setpoint, 20.0, "Heating setpoint should be 20°C");
    assert_eq!(model.model.cooling_setpoint, 27.0, "Cooling setpoint should be 27°C");
    assert_eq!(model.model.window_u_value, 3.0, "Window U-value should be 3.0 W/m²K");
}
...
fn test_case_600_simulation_performance() {
    ...
    println!("Avg time per hour: {:.2}ms", duration.as_millis() as f64 / 8760.0);
    println!("=== End ===\n");
    ...
    assert!(duration.as_secs() < 60,
            "Full-year simulation should complete in reasonable time (< 60s), got {:?}", duration);
}

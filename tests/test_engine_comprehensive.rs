use fluxion::ai::surrogate::{SurrogateManager, SurrogateOpsBox};
...
use fluxion::sim::engine::{IdealHVACController, StepParameters, ThermalModel};
...
fn test_night_ventilation_activation() {
    ...
    let step_params_inactive = StepParameters {
        use_ai: false,
        surrogates: SurrogateOpsBox::new(surrogates.clone()),
...
#[test]
fn test_thermal_mass_integration() {
    let mut model = ThermalModel::<VectorField>::new(1);
    let surrogates = SurrogateManager::default();

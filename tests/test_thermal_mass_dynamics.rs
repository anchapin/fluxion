...
fn conductance_is_valid(h_tr: f64, h_tr_op: f64, name: &str) -> bool {
    ...
    if !is_positive {
        println!(
            "WARNING: {} is negative ({:.2}), violates thermodynamics",
            name, h_tr
        );
    }
    if !is_reasonable {
        println!(
            "WARNING: {} is outside reasonable range (0.1x to 10x of h_tr_op)",
            name
        );
        println!(
            "  h_tr_op = {:.2}, h_tr = {:.2}, ratio = {:.2}x",
            h_tr_op,
            h_tr,
            h_tr / h_tr_op
        );
...
fn test_h_tr_ms_calculation() {
    ...
    println!(
        "✅ Test 1 PASSED: h_tr_ms = h_ms × A_m = {:.2} × {:.2} = {:.2} W/K",
...
fn test_h_tr_em_within_reasonable_range() {
    ...
    println!(
        "✅ Test 3 PASSED: h_tr_em ({:.2} W/K) within reasonable range",
        h_tr_em
    );
    println!(
        "   Range: [{:.2} W/K, {:.2} W/K] (0.1x to 10x of h_tr_op)",
...
fn test_conductance_values_are_finite_and_positive() {
    ...
    for (i, config) in configs.iter().enumerate() {
        ...
        if h_tr_em.is_finite() {
            ...
            println!(
                "Test case {} (Cm={:.0} kJ/K): h_tr_ms={:.1} W/K, h_tr_em={:.1} W/K ✓",
                i,
                config.cm / 1000.0,
                h_tr_ms,
                h_tr_em
            ...
            println!(
                "Test case {} (Cm={:.0} kJ/K): h_tr_ms={:.1} W/K, h_tr_em=NaN (invalid config) ⚠️",
                i,
                config.cm / 1000.0,
                h_tr_ms
            );
...
fn test_edge_case_very_small_h_tr_ms_term() {
    ...
    if h_tr_em.is_finite() {
        ...
        println!(
            "Edge case (small h_tr_ms): h_tr_ms={:.2} W/K, h_tr_em={:.2} W/K ✓",
            h_tr_ms, h_tr_em
        );
    } else {
        println!(
            "Edge case (small h_tr_ms): h_tr_ms={:.2} W/K, h_tr_em=NaN (invalid config) ✓",
            h_tr_ms
        );
    }
}
...
fn test_edge_case_very_large_h_tr_ms_term() {
    ...
    assert!(
        ...
        "h_tr_ms should be finite and positive"
    );
    ...
    println!(
        "Edge case (large h_tr_ms): h_tr_ms={:.2} W/K, h_tr_em={:.2} W/K ✓",
        h_tr_ms, h_tr_em
    );
}
...
fn test_case_900_conductance_values() {
    ...
    let h_tr_em = if reciprocal_diff > 0.0 {
        1.0 / reciprocal_diff
    } else {
        f64::NAN
    };
    ...
    println!(
        "Exterior-to-Surface Conductance (h_tr_op): {:.2} W/K",
        h_tr_op
    );
    println!();
    ...
    if h_tr_em.is_finite() {
        ...
        assert!(
            conductance_is_valid(h_tr_em, h_tr_op, "h_tr_em"),

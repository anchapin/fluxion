...
fn test_beam_to_floor_simulation_impact() {
    ...
    model.solar_gains = VectorField::from_scalar((beam_gain_watts + diffuse_gain_watts) / zone_area, 1);
    ...
    model_low.solar_gains = VectorField::from_scalar((beam_gain_watts + diffuse_gain_watts) / zone_area, 1);
    ...
    println!("TM Increase (90% fraction): {:.4} K", tm_increase_high_fraction);
    println!("TM Increase (10% fraction): {:.4} K", tm_increase_low_fraction);
    
    // With 90% fraction, the mass should heat up significantly more
    assert!(tm_increase_high_fraction > tm_increase_low_fraction * 2.0, 

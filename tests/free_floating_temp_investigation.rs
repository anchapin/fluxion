...
fn test_case_900_free_floating_temp_analysis() {
    ...
    println!("  Thermal capacitance (C): {:.2} J/K", thermal_capacitance_5r1c);
    ...
    println!("  Total mass conductance: {:.2} W/K", total_mass_conductance_5r1c);
    println!("  Time constant (τ): {:.2} hours", time_constant_5r1c);
    println!();

    println!("Heat Flow Pathways (from mass):");
    println!("  To exterior: h_tr_em = {:.2} W/K ({:.1}%)",
             h_tr_em_5r1c, (h_tr_em_5r1c / total_mass_conductance_5r1c) * 100.0);
    println!("  To surface: h_tr_ms = {:.2} W/K ({:.1}%)",
             h_tr_ms_5r1c, (h_tr_ms_5r1c / total_mass_conductance_5r1c) * 100.0);
    println!();
    ...
    println!("Thermal Mass Parameters:");
    println!("  Envelope capacitance (C_envelope): {:.2} J/K ({:.1}%)",
             cm_envelope, (cm_envelope / total_cm_6r2c) * 100.0);
    println!("  Internal capacitance (C_internal): {:.2} J/K ({:.1}%)",
             cm_internal, (cm_internal / total_cm_6r2c) * 100.0);
    println!("  Total capacitance: {:.2} J/K", total_cm_6r2c);
    println!();

    println!("Coupling Parameters:");
    println!("  h_tr_em (exterior -> envelope mass): {:.2} W/K", h_tr_em_envelope);
    println!("  h_tr_ms (envelope mass -> surface): {:.2} W/K", h_tr_ms_envelope);
    println!("  h_tr_me (envelope mass <-> internal mass): {:.2} W/K", h_tr_me);
    println!("  h_tr_is (surface -> interior): {:.2} W/K", model_6r2c.h_tr_is.as_ref()[0]);
    println!();

    // Calculate time constants for both masses
    let time_constant_envelope = cm_envelope / ((h_tr_em_envelope + h_tr_ms_envelope + h_tr_me) * 3600.0);
    let time_constant_internal = cm_internal / (h_tr_me * 3600.0);

    println!("Time Constants:");
    println!("  Envelope time constant (τ_envelope): {:.2} hours", time_constant_envelope);
    println!("  Internal time constant (τ_internal): {:.2} hours", time_constant_internal);
    println!();

    println!("Heat Flow Pathways:");
    println!("  Envelope mass -> exterior: h_tr_em = {:.2} W/K ({:.1}%)",
             h_tr_em_envelope, (h_tr_em_envelope / (h_tr_em_envelope + h_tr_ms_envelope + h_tr_me)) * 100.0);
    println!("  Envelope mass -> surface: h_tr_ms = {:.2} W/K ({:.1}%)",
             h_tr_ms_envelope, (h_tr_ms_envelope / (h_tr_em_envelope + h_tr_ms_envelope + h_tr_me)) * 100.0);
    println!("  Envelope <-> internal mass: h_tr_me = {:.2} W/K ({:.1}%)",
             h_tr_me, (h_tr_me / (h_tr_em_envelope + h_tr_ms_envelope + h_tr_me)) * 100.0);
    println!();
    ...
    if h_tr_em_ms_ratio_5r1c < 0.1 {
        println!("❌ 5R1C h_tr_em/h_tr_ms ratio too low: {:.4} < 0.1", h_tr_em_ms_ratio_5r1c);
    ...
    if avg_temp_5r1c < 15.0 {
        println!("❌ 5R1C temperature too low during winter: {:.2}°C < 15°C", avg_temp_5r1c);
        println!("   HVAC must run constantly to maintain 20°C setpoint");
        println!("   High ΔT = 20 - {:.2} = {:.2}°C causes high HVAC demand",
                 avg_temp_5r1c, 20.0 - avg_temp_5r1c);
    }

    if avg_temp_6r2c > avg_temp_5r1c {
        println!("✓ 6R2C temperature improved: {:.2}°C > 5R1C {:.2}°C", avg_temp_6r2c, avg_temp_5r1c);
        ...
        println!("⚠ 6R2C temperature not improved: {:.2}°C (need further investigation)", avg_temp_6r2c);
    }

...
fn test_case_900ff_profile_diagnostic() {
    ...
    let mut file = File::create("case_900ff_profile_hourly.csv").expect("Failed to create CSV file");
    writeln!(file, "hour,outdoor_temp,air_temp,mass_temp").expect("Failed to write header");
    ...
    for hour in 0..8760 {
        let weather_data = weather.get_hourly_data(hour).expect("Failed to get weather data");
        model.weather = Some(weather_data.clone());
        
        let outdoor_temp = weather_data.dry_bulb_temp;
        model.step_physics(hour, outdoor_temp, 3600.0);

        // Accessing first zone's temperatures
        let air_temp = model.temperatures[0];
        let mass_temp = model.mass_temperatures[0];

        writeln!(file, "{},{:.4},{:.4},{:.4}", hour, outdoor_temp, air_temp, mass_temp).expect("Failed to write row");

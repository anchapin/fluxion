...
proptest! {
    ...
    fn prop_energy_conservation(
        ...
        model.heating_setpoint = hvac_setpoint - 1.0;
        model.cooling_setpoint = hvac_setpoint + 1.0;
...
proptest! {
    ...
    fn prop_temperature_bounds(
        ...
        model.temperatures = VectorField::new(vec![initial_temp; num_zones]);
        model.mass_temperatures = VectorField::new(vec![initial_temp; num_zones]);

        // Apply random load
        let load = load_low.clamp(-1000.0, 1000.0);
        model.loads = VectorField::new(vec![load; num_zones]);
...
proptest! {
    ...
    fn prop_conductance_consistency(
        ...
        prop_assert!(model.window_u_value >= 0.1 && model.window_u_value <= 5.0,
        ...
        let window_ratio = model.window_ratio[0];
...
fn calculate_thermal_energy(model: &ThermalModel<VectorField>) -> f64 {
    ...
    for i in 0..model.num_zones {
        ...
        let air_density = model.air_density[i];
        let heat_capacity = model.heat_capacity[i];
        let zone_volume = model.zone_volume[i];
        let thermal_capacitance = model.thermal_capacitance[i];

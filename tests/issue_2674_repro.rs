...
fn pins_default_schema_succeeds_through_step_91() {
    ...
    let output = run_simulation(&schema, 1, false, ThermalSelector::default(), "issue_2674_fix").expect(
...
fn captures_stable_temperature_trace() {
    let schema = default_schema_v1();
    let output =
        run_simulation(&schema, 1, false, ThermalSelector::default(), "issue_2674_trace").expect("in-process sim must succeed");

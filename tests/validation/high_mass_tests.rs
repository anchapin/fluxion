...
use crate::validation::high_mass::test_cases::{
    create_high_mass_validation_cases, HighMassValidationCase, ValidationTolerance,
    ValidationStatus
};
...
fn test_thermal_mass_diagnostics_generation() -> Result<()> {
    ...
    assert!(report.effective_capacitance > 100.0, "Effective capacitance should be > 100 kJ/m²K for heavyweight");
    assert!(report.time_constant > 5.0, "Time constant should be > 5 hours for heavyweight");
    assert!(report.damping_factor > 0.0 && report.damping_factor < 1.0, "Damping factor should be between 0 and 1");
    assert!(!report.classification.is_empty(), "Classification should not be empty");
    
    // For heavyweight construction, we expect Heavy or VeryHeavy classification
    assert!(report.classification == "Heavy" || report.classification == "VeryHeavy", 
...
fn test_construction_type_physics_application() -> Result<()> {
    ...
    for (construction_type, expected_capacitance, expected_time_constant, expected_damping) in test_cases.iter() {
        let props = construction_type.thermal_mass_properties();
        
        // Allow some tolerance for floating point comparisons
        assert!((props.effective_capacitance - *expected_capacitance).abs() < 1.0,
                "Effective capacitance mismatch for {:?}", construction_type);
        assert!((props.time_constant - *expected_time_constant).abs() < 0.5,
                "Time constant mismatch for {:?}", construction_type);
        assert!((props.damping_factor - *expected_damping).abs() < 0.05,
                "Damping factor mismatch for {:?}", construction_type);
    }
    ...
    assert!(props.effective_capacitance > 0.0, "Custom construction should have positive effective capacitance");
    assert!(props.time_constant > 0.0, "Custom construction should have positive time constant");
    assert!(props.damping_factor > 0.0 && props.damping_factor < 1.0, 
...
fn test_high_mass_validation_cases_creation_and_execution() -> Result<()> {
    ...
    assert_eq!(cases.len(), 3, "Should have 3 predefined high-mass validation cases");
    
    let case_ids: Vec<String> = cases.iter().map(|c| c.case_id.clone()).collect();
    assert!(case_ids.contains(&"600".to_string()), "Should include Case 600");
    assert!(case_ids.contains(&"650".to_string()), "Should include Case 650");
    assert!(case_ids.contains(&"900".to_string()), "Should include Case 900");
    ...
    for case in cases {
        ...
        match case.case_id.as_str() {
            "600" | "900" => {
                assert!(matches!(case.building_config.construction_type, ConstructionType::HeavyWeight),
                        "Case {} should use HeavyWeight construction", case.case_id);
            },
            "650" => {
                assert!(matches!(case.building_config.construction_type, ConstructionType::MediumWeight),
        ...
        assert!(!layers.is_empty(), "Case {} should have construction layers", case.case_id);
        
        // Verify that we can run thermal mass diagnostics
        let heat_loss_coefficient = crate::validation::high_mass::test_cases::calculate_heat_loss_coefficient(
            case.building_config.u_value,
            case.building_config.floor_area,
            case.building_config.window_wall_ratio,
        );
        
        let diagnostics = ThermalMassDiagnostics::with_construction_layers(layers, heat_loss_coefficient);
        let report = diagnostics.analyze();
        
        assert!(report.effective_capacitance > 0.0, "Case {} should have positive effective capacitance", case.case_id);
        assert!(report.time_constant > 0.0, "Case {} should have positive time constant", case.case_id);
        assert!(report.damping_factor > 0.0 && report.damping_factor < 1.0, 
                "Case {} damping factor should be between 0 and 1", case.case_id);
    }

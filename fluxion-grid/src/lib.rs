//! Joint thermal-electrical convergence solver for grid-interactive building models.
//!
//! This module implements iterative coupling between thermal (zone temperatures, HVAC loads)
//! and electrical (bus voltages, power flows) systems through a heat pump coupling model.
//!
//! # Architecture
//!
//! The joint convergence loop follows this iteration pattern:
//! 1. **Solve thermal** → compute zone temperatures and HVAC thermal loads
//! 2. **Compute heat pump load** → convert thermal load to electrical power via COP
//! 3. **Solve electrical** → compute bus voltages and power flows
//! 4. **Update COP** → recalculate coefficient of performance based on electrical state
//! 5. **Check convergence** → repeat until both thermal residual AND electrical mismatch < tolerance
//!
//! # Example
//!
//! ```
//! use fluxion_grid::{JointConvergenceSolver, ThermalModel, ElectricalNetwork, ThermalElectricalCoupler};
//!
...
//! assert!(result.converged, "Joint solver should converge");
//! ```

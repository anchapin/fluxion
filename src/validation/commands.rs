...
pub fn update_references_ext(url: Option<&str>, output_path: &Path) -> Result<()> {
    match url {
        Some(remote_url) => {
            // User-facing CLI output: the `fluxion` binary does not install a
            // tracing subscriber, so these messages must go to stdout/stderr
            // directly (Issue #3952 — `references update` was silent).
            println!("Fetching reference data from {}...", remote_url);

...
impl ValidationResults {
    ...
    pub fn calculate_annual_totals(&mut self) {
        ...
        self.peak_heating = *self.hourly_heating.iter().fold(&0.0, |a, &b| if b > *a { &b } else { a });
        self.peak_cooling = *self.hourly_cooling.iter().fold(&0.0, |a, &b| if b > *a { &b } else { a });
    }
}
...
pub fn compare_results(
    ...
    let annual_heating_diff = calculate_percentage_diff(fluxion.annual_heating, reference.annual_heating);
    let annual_cooling_diff = calculate_percentage_diff(fluxion.annual_cooling, reference.annual_cooling);
    let avg_percentage_diff = (annual_heating_diff + annual_cooling_diff) / 2.0;
    
    // Calculate max deviation
    let max_deviation = fluxion.hourly_temperatures.iter()
    ...
    let details = fluxion.hourly_temperatures.iter()
        .zip(reference.hourly_temperatures.iter())
        .enumerate()
...
pub fn calculate_rmse(fluxion: &[f64], reference: &[f64]) -> f64 {
    ...
    let sum_squared: f64 = fluxion.iter()
        .zip(reference.iter())
        .map(|(&f, &r)| {
            let diff = f - r;
            diff * diff

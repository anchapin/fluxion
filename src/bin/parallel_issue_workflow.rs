...
impl RankedIssue {
    fn calculate_score(issue: &GitHubIssue, priority_labels: &[String]) -> (f64, Option<String>) {
        ...
        if !issue.created_at.is_empty() {
            if let Ok(created) = chrono::NaiveDate::parse_from_str(&issue.created_at[..10], "%Y-%m-%d") {
                let today = chrono::Utc::now().date_naive();
...
fn fetch_issues(repo: &str, limit: usize) -> Result<Vec<GitHubIssue>> {
    println!("Fetching issues from {}/{}...", repo.split('/').next().unwrap_or(""), repo.split('/').nth(1).unwrap_or(""));
    ...
    let issues: Vec<GitHubIssue> = serde_json::from_str(&output)
        .map_err(|e| anyhow::anyhow!("JSON parse error at line {}, column {}: {}", e.line(), e.column(), e))?;
...
fn get_issue_details(repo: &str, issue_number: u64) -> Result<GitHubIssue> {
    ...
    let issue: GitHubIssue = serde_json::from_str(&output)
...
"#,
        ...
        labels = issue.labels.iter().map(|l| format!("- {}", l.name)).collect::<Vec<_>>().join("\n"),
        milestone = issue.milestone.as_ref().map(|m| m.title.as_str()).unwrap_or("None"),
        url = issue.url,
...
async fn run_workflow(args: Args) -> Result<()> {
    ...
    if let Err(e) = gh_exec(&["auth", "status"]) {
        anyhow::bail!("gh CLI not authenticated: {}. Run 'gh auth login' first.", e);
    }
    ...
    for (idx, ranked) in top_issues.iter().enumerate() {
        ...
        println!("   Comments: {}, Reactions: {}",
            ranked.issue.comments.len(),
            ranked.issue.reactions.thumbs_up() + ranked.issue.reactions.rocket() + ranked.issue.reactions.hooray()
        );
    }
    println!();
...
mod tests {
    ...
    fn test_ranked_issue_score_calculation() {
        let issue = GitHubIssue {
            ...
            labels: vec![
                GitHubLabel { name: "critical".to_string() },
            ...
            reactions: Reactions {
                reaction_groups: vec![
                    ReactionGroup { content: "THUMBS_UP".to_string(), count: 10 },
                    ReactionGroup { content: "THUMBS_DOWN".to_string(), count: 0 },
                    ReactionGroup { content: "EYES".to_string(), count: 2 },
                    ReactionGroup { content: "ROCKET".to_string(), count: 3 },
                    ReactionGroup { content: "HOORAY".to_string(), count: 1 },
                ],
            },
    ...
    fn test_branch_name_generation() {
        let issue = GitHubIssue {
            ...
            reactions: Reactions {
                reaction_groups: vec![
                    ReactionGroup { content: "THUMBS_UP".to_string(), count: 0 },
                    ReactionGroup { content: "THUMBS_DOWN".to_string(), count: 0 },
                    ReactionGroup { content: "EYES".to_string(), count: 0 },
                    ReactionGroup { content: "ROCKET".to_string(), count: 0 },
                    ReactionGroup { content: "HOORAY".to_string(), count: 0 },
                ],
            },
            state: "open".to_string(),
            url: "".to_string(),

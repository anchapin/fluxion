...
fn main() {
    ...
    println!(
        "   Case 940 (setback):  {:.2e} J/K",
        model_940.thermal_capacitance[0]
    );
    println!(
        "   Case 900 (baseline): {:.2e} J/K",
        model_900.thermal_capacitance[0]
    );
    println!(
        "   Ratio (940/900):     {:.2}x",
        model_940.thermal_capacitance[0] / model_900.thermal_capacitance[0]
    );
    ...
    println!(
        "   Case 940 exceeds threshold: {}",
        model_940.thermal_capacitance[0] > high_mass_threshold
    );
    println!(
        "   Case 900 exceeds threshold: {}",
        model_900.thermal_capacitance[0] > high_mass_threshold
    );
    ...
    println!(
        "   h_tr_em (exterior→mass):  {:.2} W/K",
        model_940.h_tr_em.as_ref()[0]
    );
    println!(
        "   h_tr_ms (mass→surface):  {:.2} W/K",
        model_940.h_tr_ms.as_ref()[0]
    );
    println!(
        "   h_tr_is (surface→interior): {:.2} W/K",
        model_940.h_tr_is.as_ref()[0]
    );
    println!(
        "   h_tr_w  (exterior→interior via windows): {:.2} W/K",
        model_940.h_tr_w.as_ref()[0]
    );
    println!(
        "   h_ve    (ventilation):      {:.2} W/K",
        model_940.h_ve.as_ref()[0]
    );
    ...
    println!(
        "   Case 940 coupling ratio (h_tr_em / h_tr_ms): {:.3}",
        coupling_ratio_940
    );
    println!("   Target ratio: 0.1");
    println!(
        "   Status: {}",
        if coupling_ratio_940 >= 0.1 {
            "✅ ADEQUATE"
        } else {
            "❌ TOO WEAK"
        }
    ...
    println!(
        ...
        model_940.h_tr_em_heating.as_ref()[0]
    );
    println!(
        "   h_tr_em_cooling: {:.2} W/K",
        model_940.h_tr_em_cooling.as_ref()[0]
    );
    println!(
        "   Heating coupling ratio: {:.3}",
        model_940.h_tr_em_heating.as_ref()[0] / model_940.h_tr_ms.as_ref()[0]
    );
    println!(
        "   Cooling coupling ratio: {:.3}",
        model_940.h_tr_em_cooling.as_ref()[0] / model_940.h_tr_ms.as_ref()[0]
    );

    // Time constant
    let tau_940 = model_940.thermal_capacitance[0]
        / (model_940.h_tr_ms.as_ref()[0] + model_940.h_tr_em.as_ref()[0]);
    let tau_900 = model_900.thermal_capacitance[0]
        / (model_900.h_tr_ms.as_ref()[0] + model_900.h_tr_em.as_ref()[0]);
    ...
    println!(
        "   h_tr_em (after correction): {:.2} W/K",
        model_940.h_tr_em.as_ref()[0]
    );
    println!(
        "   h_tr_em_heating (after):    {:.2} W/K",
        model_940.h_tr_em_heating.as_ref()[0]
    );
    println!(
        "   h_tr_em_cooling (after):    {:.2} W/K",
        model_940.h_tr_em_cooling.as_ref()[0]
    );
    ...
    for hour in [0, 6, 7, 12, 23] {
        ...
        let status = if heating_sp == 10.0 {
            "SETBACK"
        } else {
            "NORMAL"
        };
        println!(
            "   Hour {:2}: Heating SP = {:.1}°C, Cooling SP = {:.1}°C [{}]",
            hour, heating_sp, cooling_sp, status
        );
    }

//! Core model bindings: the [`Model`] pyclass and the snapshot types.
//!
//! This module holds the pieces of the former monolithic
//! `src/python/model_bindings.rs` (Issue #1812) that describe the building
//! model itself:
//!
//! - [`Model`] — the top-level Python entrypoint (Issue #2493),
//! - the snapshot pyclasses [`PyOrientation`], [`PyShadingDevice`],
//!   [`PyShadingType`], [`PyMaterial`], [`PySurface`], [`PyZone`],
//! - the model<->snapshot helpers (`zone_from_model`,
//!   `all_surfaces_from_model`, `all_zones_from_model`, `surface_to_wall`,
//!   `reshape_surfaces_for_model`),
//! - [`populate_default_model_physics`] (Issue #2806).
//!
//! HVAC snapshots live in [`super::hvac`]; the batch-evaluation entrypoints
//! (`get_parameter_bounds`, `validate_parameters_py`) keep one-line
//! `#[pymethods]` shims on [`Model`] here (PyO3 0.29 rejects a second
//! `#[pymethods] impl Model` block in another module with E0119) while their
//! logic lives in [`super::batch`]. The Python class `fluxion.Model` is
//! unchanged.
//!
...
//!
//! - Each [`PyZone`], [`PySurface`], [`PyMaterial`] holds
...
//!   (The [`PyHVACSystem`](super::hvac::PyHVACSystem) snapshot follows the same
//!   story; it lives in [`super::hvac`].)
...
impl Model {
    ...
    fn get_parameter_bounds(&self) -> ParameterBounds {
        super::batch::get_parameter_bounds()
    }
...
mod tests {
    ...
    //! - the model<->snapshot helpers (`zone_from_model`,
    //!   `all_surfaces_from_model`, `all_zones_from_model`,
    //!   `reshape_surfaces_for_model`),
    //! - `populate_default_model_physics` (Issue #2806) and the analytical
    //!   no-divergence regressions.

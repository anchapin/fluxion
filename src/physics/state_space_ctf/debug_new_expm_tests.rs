...
fn test_ctf_convergence_random_assemblies() {
    ...
    runner
        .run(&any_ctf_material_params(), |(thickness, k, rho, cp)| {
            ...
            let nodes_per_layer = compute_nodes_per_layer(std::slice::from_ref(&layer), timestep);
            let n: usize = nodes_per_layer.iter().sum();
    ...
    runner
        .run(
            ...
            |(t1, k1, r1, cp1, t2, k2, r2, cp2, t3, k3, r3, cp3)| {
                ...
                let result = std::panic::catch_unwind(|| compute_state_space_ctf(layers, timestep));
                ...
                if (dc_gain - u_filmed).abs() / u_filmed > 0.25 {
                    eprintln!(
                        ...
                        ((dc_gain - u_filmed).abs() / u_filmed * 100.0),
                        dc_gain,
                        u_filmed
                    );
                }

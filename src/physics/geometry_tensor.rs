...
/// Phase 1b placeholder for #1461's full ThermalManifold. This stub only carries per-zone gauge_connection boundary vectors so GaugeSolver shadow mode can compile until Phase 1a replaces it.
#[derive(Debug, Clone)]
pub struct ThermalManifold {
    pub geometry: GeometryTensor,
    pub gauge_connection: Vec<VectorField>,
}

pub const GAUGE_CONNECTION_COMPONENTS: usize = 2;
pub const GAUGE_CONNECTION_SOLAR_INDEX: usize = 0;
pub const GAUGE_CONNECTION_OUTDOOR_TEMP_INDEX: usize = 1;
...
impl ThermalManifold {
    pub fn new(num_zones: usize) -> Self {
        ...
        let mut geometry = GeometryTensor::new();
    ...
    pub fn from_geometry(geometry: GeometryTensor) -> Self {
        let num_zones = geometry.num_zones();
        assert!(
            num_zones <= MAX_ZONES,
            "ThermalManifold zone count exceeds MAX_ZONES"
        );
    ...
    pub fn gauge_connection(&self, zone_index: usize) -> Option<&VectorField> {

...
// Re-export the zone-count envelope policy (Issue #3731, hoisted to the
// `fluxion-core` dependency-light leaf crate by Issue #3871 to close the
// residual `sim -> physics` edge admitted by PR #3869 for the typed
// `ZoneCountPolicy` import under `src/sim/thermal_model_core/mod.rs:9`).
//
// The canonical home is now `fluxion_core::zone_count_policy`. This file
// keeps the legacy paths (`crate::physics::geometry_tensor::MAX_ZONES`,
// `crate::physics::geometry_tensor::ZoneCountPolicy`) working as thin
// re-exports so:
//   * `tests/all_tests/gauge_validation_case_900.rs:87` (which imports
//     `fluxion::physics::geometry_tensor::{ManifoldIndex, ThermalManifold,
//     MAX_ZONES}`) keeps compiling unchanged.
//   * The `ZONE_COORDS_DIMS` / `ADJACENCY_MATRIX_DIMS` /
//     `ZONE_PROPERTIES_DIMS` constants below can still reference `MAX_ZONES`
//     by short name within this module.
//
// New code should import from `fluxion_core::zone_count_policy` directly.

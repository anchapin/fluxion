...
// =============================================================================
// Zone-count envelope policy (Issue #3731)
//
...
// zone count into the appropriate tier, and emits a typed `PhysicsError`
// for the beyond-envelope case — matching the convention every other gauge
// pre-check uses (`PhysicsError::initialization`). Callers MUST go through
// this wrapper: a code path that bypasses it (e.g. constructs a
// `ThermalManifold::new(num_zones)` directly) will hit the
// `assert!(num_zones <= MAX_ZONES, ...)` in `gauge_solver.rs` and panic —
// that path is intentionally left in place as the secondary, deep-defense
// check, but it should be unreachable from the public gauge API now that
// `ZoneCountPolicy::check_gauge` is wired into both `enable_gauge_solver`
// and `enable_gauge_solver_multi_zone`.
//
// Tier semantics:
//
...
// `check_gauge` method returns `Err(PhysicsError)`). The wrapper is the
...
///   beyond-envelope count as a typed [`PhysicsError::initialization`]
///   at the gauge initialization entry point (single- and multi-zone).
///
/// Issue #3731.
...
impl ZoneCountPolicy {
    ...
    /// * `Empty` → `Err(PhysicsError::initialization(...))`.
    /// * `BeyondEnvelope` → `Err(PhysicsError::initialization(...))` with
    ///   a message that names the spec's `num_zones`, the gauge envelope
    ///   constant `MAX_ZONES`, and the §LIMIT-21 / Issue #3731 / Phase-1b
    ///   geometry rework that owns growing the envelope.
    ///
    /// Callers in the gauge initialization entry points MUST invoke
    /// this method (or `ZoneCountPolicy::for_count` + tier switch) — the
    /// typed error is what surfaces a beyond-envelope count as a clean
    /// `Err`, instead of the deep `assert!` panic in the shadow-mode
    /// `ThermalManifold::new(num_zones)` (`src/physics/gauge_solver.rs`).
    pub fn check_gauge(&self) -> Result<(), crate::physics::solver_trait::PhysicsError> {
        use crate::physics::solver_trait::PhysicsError;
        match self.tier {
            ZoneCountTier::Standard | ZoneCountTier::AtCapacity => Ok(()),
            ZoneCountTier::Empty => Err(PhysicsError::initialization(
                "Zone-count policy rejects num_zones = 0 (Empty tier): \
                 the gauge selector requires at least one thermal zone. \
                 See `geometry_tensor::ZoneCountPolicy` (Issue #3731).",
            )),
            ZoneCountTier::BeyondEnvelope => Err(PhysicsError::initialization(&format!(
                "Zone-count policy rejects num_zones = {} (> MAX_ZONES = {}): \

mod tests {
    ...
    // ---------------------------------------------------------------------
    // floor_u_value_override: data-driven ground-coupling U-value
    // ---------------------------------------------------------------------
    // The ASHRAE 140 Case 195 solid-conduction test specifies its floor
    // U-value (0.039 W/m²K) as spec data instead of a `case_id == "195"`
    // hard-code; the old 900-series ×1.2 h_tr_floor multiplier is removed
    // (dead code — 900-series auto-promotes to 9R4C, which never consumes
    // the lumped h_tr_floor).

    #[test]
    fn case_195_spec_carries_floor_u_override() {
        let spec = crate::validation::ashrae_140_cases::CaseBuilder::case_195_solid_conduction();
        assert!(
    ...
    fn from_spec_900_series_has_no_floor_multiplier() {
        ...
        let spec = crate::validation::ashrae_140_cases::ASHRAE140Case::Case900.spec();

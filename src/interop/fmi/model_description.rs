...
//! FMI 2.0 model-description XML generation and parsing.
//!
//! This module owns the [`modelDescription.xml`] half of Fluxion's FMI 2.0
//! interop:
//!
//! * **Generation (export)** — [`FmiExporter`] and [`FfdFmuExporter`] build a
//!   valid FMI 2.0 `modelDescription.xml` (via `quick-xml`) and package it
//!   into a `.fmu` ZIP archive, together with the variable-name templates
//!   ([`FmiVariables`], [`ZoneVariables`], [`FfdFmuVariables`]) and the XML
//!   helpers (`write_real_variable`, `format_float`, `generation_timestamp`,
//!   …).
//! * **Parsing (import)** — [`FmiImporter`] reads a `.fmu` archive back,
//!   parses its `modelDescription.xml` into [`ImportedModelDescription`]
//!   (XXE-hardened per issue #3591), and rebuilds the [`ThermalModel`] via
//!   [`import_fmu`].
//!
//! The FMU lifecycle (instantiation, initialization, stepping, reset) lives
//! in [`lifecycle`](super::lifecycle), and the per-timestep value-transfer
//! structs live in [`marshaling`](super::marshaling).
//!
